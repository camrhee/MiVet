...
function VetProfiles() {
  ...
  const [serviceTypes, setServiceTypes] = useState({
    arrayOfServiceTypes: [],
    serviceTypesComponents: [],
  });

  useEffect(() => {
    vetProfilesService
      .getAllPaginated(0, pageData.pageSize)
  ...
  const mapServiceTypes = (aServiceType) => {
    return (
      <option key={aServiceType.id} value={aServiceType.id}>
        {aServiceType.name}
      </option>
    );
  };
  ...
  const onLookUpSuccess = (data) => {
    let arrayOfServiceTypes = data.item.servicetypes;

    setServiceTypes((prevState) => {
      const pd = { ...prevState };
      pd.arrayOfServiceTypes = arrayOfServiceTypes;
      pd.serviceTypesComponents = arrayOfServiceTypes.map(mapServiceTypes);
      return pd;
    });
  };
  ...
  const onGetAllProfileChange = (page) => {
    ...
    vetProfilesService
      .getAllPaginated(page - 1, pageData.pageSize)
      .then(onGetProfilesSucess)
      .catch(onGetProfilesError);
  };
  ...
  const onSearchProfileChange = (page) => {
    ...
    vetProfilesService
      .search(page - 1, pageData.pageSize, query)
  ...
  const onSearchProfilesClick = (e) => {
    ...
    vetProfilesService
      .search(
        ...
        pageData.pageSize,
        query,
        selectedServiceType ? parseInt(selectedServiceType.serviceType) : null
      )
      .then(onSearchProfilesClickSuccess)
      .catch(onSearchProfilesClickError);
    setShow(false);
      ...
      <Tab.Container defaultActiveKey="grid">
        ...
        <div className="mb-4 row">
          <Col className="align-vertical-children-vetprofile">
            <div className="align-search-menu-vetprofile">
              <select
              ...
              >
                ...
                {serviceTypes.serviceTypesComponents}
              </select>
            </div>
            <Form.Control
